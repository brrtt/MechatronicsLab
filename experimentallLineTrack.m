...
rm.start();
while(true)
    ls = readInputDeviceREADY_RAW(myev3,1,0,1);
    ms = readInputDeviceREADY_RAW(myev3,2,0,1);
    rs = readInputDeviceREADY_RAW(myev3,4,0,1);
    us = range_converter(readInputDeviceREADY_RAW(myev3,3,0,1),66,166,2,20);
    disp("Distance: "+us);
    if(us < us_threshold)
        lm.stop();
        rm.stop();
